...
var events = Event.startEvetn

class ViewController: UIViewController {
    
    @IBOutlet var tableView: UITableView!
    // Идентификатор кастомной ячейки. Задается ее название
...
extension ViewController : UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //cell.nameEvent.text = event.name
        cell.locationEvent.text = event.location
        cell.maxRegistredGuests.text = String(event.maxNumberOfGuest)
        cell.currentRrgistredGuests.text = String(event.currentNumberOfRegistGuest)
        cell.typeEvemtCell.text = event.type.rawValue
        cell.durationCell.text = event.duration
        cell.startingTimeCell.text = event.startTime?.description
        cell.endTimeCell.text = event.endTime?.description
        cell.descriptionCell.text = event.description
